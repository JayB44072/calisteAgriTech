...
function Toggle({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/30 ${checked ? 'bg-emerald-600' : 'bg-gray-300 dark:bg-slate-600'}`}
    >
    ...
    <div className="relative space-y-6 animate-fade-in max-w-2xl">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
        <SectionCard title="Apparence" icon={Sun}>
            ...
            <div className="flex bg-gray-100 dark:bg-slate-700 rounded-xl p-1">
              {(['fr', 'en'] as const).map(l => (
                <button key={l} onClick={() => setLang(l)}
                  className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all ${lang === l ? 'bg-white dark:bg-slate-600 text-emerald-700 dark:text-emerald-400 shadow-sm' : 'text-gray-500 dark:text-slate-400'}`}>
                  {l.toUpperCase()}
                </button>
              ))}
            </div>
            ...
            <div className="flex bg-gray-100 dark:bg-slate-700 rounded-xl p-1">
              {themeOptions.map(({ id, label, icon: Icon }) => (
                <button key={id} onClick={() => { if (theme !== id) toggleTheme(); }}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${theme === id ? 'bg-white dark:bg-slate-600 text-emerald-700 dark:text-emerald-400 shadow-sm' : 'text-gray-500 dark:text-slate-400'}`}>
          ...
          <SettingRow label="Unités de mesure" description="Hectares, °C, m³">
            <div className="flex bg-gray-100 dark:bg-slate-700 rounded-xl p-1">
              <button className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white dark:bg-slate-600 text-emerald-700 dark:text-emerald-400 shadow-sm">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
        <SectionCard title="Sécurité du compte" icon={Lock}>
          {pwSuccess && (
            <div className="flex items-center gap-2 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-lg px-3 py-2 text-sm">
            ...
            {[
              ...
              <div key={key}>
                ...
                <input type="password" placeholder={placeholder}
                  ...
                  className="w-full border border-gray-200 dark:border-slate-600 rounded-xl px-4 py-2.5 text-sm bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500 transition-all"
                />
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-semibold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 px-2 py-0.5 rounded-full">Actif</span>
              </div>
              <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">5 parcelles · 10 capteurs · IA limitée</p>
            </div>
            <button onClick={() => setShowPro(true)} className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 hover:underline">
              Passer Pro →
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          ...
          >
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-3xl font-bold text-emerald-600 dark:text-emerald-400 mt-3">15 000 <span className="text-base font-normal text-gray-500">FCFA / mois</span></p>
            </div>
            ...
            <button
              onClick={() => window.open('https://wa.me/237600000000?text=Je veux passer au plan Pro CalisteAgriTech', '_blank')}
